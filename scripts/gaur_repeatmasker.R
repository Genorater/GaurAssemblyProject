#------------------------------------------------------
# Program name: gaur_repeatmasker.R
# Objective: check frozen gaur repeat masker output
#           to make plots
# Author: Lloyd Low
...
#############################################################################################
#read in GCF_003121395.1_ASM312139v1_rm.out
...
#############################################################################################
#read in GCA_014182915.1_ARS_UOA_Gaur_1_rm.out
ARS_UOA_Gaur_1 <- read_tsv("/Users/lloyd/Documents/lloyd_2019/Research/gaur/repeatmaskering/GCA_014182915.1_ARS_UOA_Gaur_1_rm.out",
                      col_names = FALSE)

#no column 16 containing *, no need to remove
names(ARS_UOA_Gaur_1) <- c("score", "divergence", "deletion", "insertion", "query_sequence", "query_begin", 
                   "query_end", "query_left", "strand","repeat", "family", "repeat_begin", 
                   "repeat_end","repeat_left","ID")

#convert query_sequence to proper name e.g. 1, 2, 3, ... X
chr1_29 <- c("CM024435.1","CM024436.1","CM024437.1","CM024438.1","CM024439.1","CM024440.1","CM024441.1",
             "CM024442.1","CM024443.1","CM024444.1","CM024445.1","CM024446.1","CM024447.1","CM024448.1",
             "CM024449.1","CM024450.1","CM024451.1","CM024452.1","CM024453.1","CM024454.1","CM024455.1",
             "CM024456.1","CM024457.1","CM024458.1","CM024459.1","CM024460.1","CM024461.1","CM024462.1",
             "CM024463.1")

chrX <- "CM024464.1"
chr1_X <- c(chr1_29,chrX)

ARS_UOA_Gaur_1$query_sequence[!(ARS_UOA_Gaur_1$query_sequence %in% chr1_X)] <- "Unplaced"

#loop thro character vector representing chromosomes to sub in "1","2",... for easier chr identification
vect1 <- c()
for (i in 1:length(chr1_29)){
  vect1 <- ARS_UOA_Gaur_1$query_sequence %in% chr1_29[i]
  ARS_UOA_Gaur_1$query_sequence[vect1] <- as.character(i)
}

ARS_UOA_Gaur_1$query_sequence[ARS_UOA_Gaur_1$query_sequence %in% chrX] <- "X"

#repeat_begin and repeat_left have silly ()
new_repeat_begin <- gsub("\\(","",ARS_UOA_Gaur_1$repeat_begin)
new_repeat_begin <- gsub("\\)","",new_repeat_begin)
new_repeat_begin <- as.integer(new_repeat_begin)

new_repeat_left <- gsub("\\(","",ARS_UOA_Gaur_1$repeat_left)
new_repeat_left <- gsub("\\)","",new_repeat_left)
new_repeat_left <- as.integer(new_repeat_left)

ARS_UOA_Gaur_1$new_repeat_begin <- new_repeat_begin
ARS_UOA_Gaur_1$new_repeat_end <- ARS_UOA_Gaur_1$repeat_end
ARS_UOA_Gaur_1$new_repeat_left <- new_repeat_left

#loop thro and add query_seq_align_len and perc_id
ARS_UOA_Gaur_1_subset <- ARS_UOA_Gaur_1 %>% 
  mutate(query_align_len = query_end - query_begin + 1, perc_id = 1 - (divergence/100)) %>%
  filter(perc_id > 0.6) %>%
  select("query_sequence","query_begin","query_end","query_left","strand","repeat","family","query_align_len")

ARS_UOA_Gaur_1_subset$assembly <- rep("ARS_UOA_Gaur_1",nrow(ARS_UOA_Gaur_1_subset))

#top align length of repeat family with highest presence
ARS_UOA_Gaur_1_subset_topRepeatLen <- ARS_UOA_Gaur_1_subset %>% 
  group_by(family) %>% summarise(total_len = sum(query_align_len)) %>% 
  arrange(desc(total_len))
...
ARS_UOA_Gaur_1_subset_noUnplaced <- ARS_UOA_Gaur_1_subset %>% filter(query_sequence != "Unplaced")
...
#                            ARS_UOA_Gaur_1_subset_noUnplaced)
# 
# #order spp
# order_spp <- c("Angus","Brahman","ARS-UCD1.2","UMD3.1.1","ARS_UOA_Gaur_1")
...
# tiff(filename = "RepeatsFamiliyByLengthGaur_with_allCattle.tiff",width = 900)
...
#for gaur paper
#all_spp_rm_subset <- rbind(umd3_1_subset,arsucd1_2_subset,angus_subset,brahman_subset)
all_spp_rm_subset <- rbind(umd3_1_subset_noUnplaced,arsucd1_2_subset_noUnplaced,
                           ARS_UOA_Gaur_1_subset_noUnplaced)

#order spp
order_spp <- c("UMD3.1.1","ARS-UCD1.2","ARS_UOA_Gaur_1")
all_spp_rm_subset$assembly <- factor(all_spp_rm_subset$assembly, 
                                     levels = order_spp)
...
#The shape of violin starts and ends with the IQR ends
tiff(filename = "RepeatsFamiliyByLengthGaur.tiff",width = 900)
bp <- ggplot(all_spp_rm_subset_violin, aes(x=assembly, y=query_align_len, group=assembly))
bp <- bp + geom_violin(aes(fill=assembly)) + guides(fill=FALSE)
bp <- bp + scale_y_log10(labels = scales::comma,breaks=c(0,5000,10000,15000,20000,25000,30000,40000,50000,60000,80000,100000,120000,150000,250000,500000))
bp <- bp + facet_grid(. ~ family) + theme_bw(base_size = 13.5) 
...
##for gaur supplementary
#Combine umd3_1_subset,arsucd1_2_subset,angus_subset,brahman_subset to look at unplaced vs in chr
all_spp_rm_subset_supp <- rbind(umd3_1_subset,arsucd1_2_subset,ARS_UOA_Gaur_1_subset)
...
#order spp
# order_spp <- c("UMD3.1.1","ARS-UCD1.2","ARS_UOA_Gaur_1")
all_spp_rm_subset_supp_summ$assembly <- factor(all_spp_rm_subset_supp_summ$assembly, 
                                     levels = order_spp)

#barplot of different repeat family count by sequence placement status
#the weakness of this figure is that it gives the count, does not say what length
#When interpreted together with the centr length, indeed the gaur centr repeats must be
#shorter and there are more of it, hence higher count.
# tiff(filename = "RepeatsFamilyGaur.tiff",width = 600)
# h <- ggplot(data = all_spp_rm_subset_supp_summ, aes(x = family, y = log10(count),fill = assembly)) + 
#   geom_bar(stat = "identity",position="dodge") + ylab(expression(log[10]*" count")) + xlab("Repeat family")
# h <- h + facet_grid(~placement) 
# h <- h + theme(plot.title = element_text(hjust = 0.5),axis.text.x = element_text(angle = 0, hjust = 0.5))
...
#dominette centromeric repeats
arsucd1_2_subset_chr_cent_alignLength <- arsucd1_2_subset %>% filter(family == "Satellite/centr")
sum(arsucd1_2_subset_chr_cent_alignLength$query_align_len)/1e6
#Dominette has 48.75535 Mb, wow!

ARS_UOA_Gaur_1_subset_chr_cent_alignLength <- ARS_UOA_Gaur_1_subset %>% filter(family == "Satellite/centr")
sum(ARS_UOA_Gaur_1_subset_chr_cent_alignLength$query_align_len)/1e6
#Gaur has 10.44335 Mb of centr repeats

##### <START> PERCENTAGE OF GAUR REPEATS in the GENOME #####
#loop gaur and calc align query length but dont filter percent id to get %repeat
ARS_UOA_Gaur_1_subset_nofilpercid <- ARS_UOA_Gaur_1 %>% 
  mutate(query_align_len = query_end - query_begin + 1, perc_id = 1 - (divergence/100)) %>%
  select("query_sequence","query_begin","query_end","query_left","strand","repeat","family","query_align_len")

#gaur repeat of all classes straight from repeat masker/genome size
sum(ARS_UOA_Gaur_1_subset_nofilpercid$query_align_len)/2717349581
#0.4778207

#loop arsucd1.2 and calc align query length but dont filter percent id to get %repeat
arsucd1_2_subset_nofilpercid <- arsucd1_2 %>% 
  mutate(query_align_len = query_end - query_begin + 1, perc_id = 1 - (divergence/100)) %>%
  select("query_sequence","query_begin","query_end","query_left","strand","repeat","family","query_align_len")

#arsucd1.2 repeat of all classes straight from repeat masker/genome size
sum(arsucd1_2_subset_nofilpercid$query_align_len)/2715853792
#0.498961
##### <END> PERCENTAGE OF GAUR REPEATS in the GENOME #####

##### <START> PERCENTAGE OF TOP TWO repeat families #####
#LINE/L1 & LINE/RTE-BovB are the top two
sum(ARS_UOA_Gaur_1_subset_topRepeatLen$total_len[1:2])/2717349581
#0.2503078
sum(arsucd1_2_subset_topRepeatLen$total_len[1:2])/2715853792
#0.2481349
##### <END> PERCENTAGE OF TOP TWO repeat families #####

##### <START> Highest repeat family in the unplaced #####
ARS_UOA_Gaur_1_subset_Unplaced <- ARS_UOA_Gaur_1_subset %>% filter(query_sequence == "Unplaced") 
ARS_UOA_Gaur_1_subset_Unplaced_topRepeatLen <- ARS_UOA_Gaur_1_subset_Unplaced %>% group_by(family) %>% 
  summarise(total_len = sum(query_align_len)) %>% 
  arrange(desc(total_len))
ARS_UOA_Gaur_1_subset_Unplaced_topRepeatLen$total_len[1]/sum(ARS_UOA_Gaur_1_subset_Unplaced_topRepeatLen$total_len)
#Gaur has 5358132 bases of Satellite/centr in the unplaced, which is the highest in terms of length.
#0.3130091 of the repeats in the unplaced are Satellite/centr type. Note that the Satellite/centr length
#is not set to be more than 10e3. 

arsucd1_2_subset_Unplaced <- arsucd1_2_subset %>% filter(query_sequence == "Unplaced")
arsucd1_2_subset_Unplaced_topRepeatLen <- arsucd1_2_subset_Unplaced %>% group_by(family) %>% 
  summarise(total_len = sum(query_align_len)) %>% 
  arrange(desc(total_len))
arsucd1_2_subset_Unplaced_topRepeatLen$total_len[1]/sum(arsucd1_2_subset_Unplaced_topRepeatLen$total_len)
#ARS-UCD1.2 has 46123208 bases of Satellite/centr in the unplaced, which is the highest in terms of length.
#0.6958715 of the repeats in the unplaced are Satellite/centr type
##### <END> Highest repeat family in the unplaced #####

##### <START> centromeric repeats #####
#toggle on-off %>% filter(query_align_len > 10e3) to get the results for paper
ARS_UOA_Gaur_1_subset_centromere <- ARS_UOA_Gaur_1_subset %>% filter(family == "Satellite/centr") %>% 
  filter(query_sequence != "Unplaced") #%>% filter(query_align_len > 10e3) 
ARS_UOA_Gaur_1_subset_centromere$query_left <- gsub("\\(","",ARS_UOA_Gaur_1_subset_centromere$query_left)
ARS_UOA_Gaur_1_subset_centromere$query_left <- gsub("\\)","",ARS_UOA_Gaur_1_subset_centromere$query_left)
ARS_UOA_Gaur_1_subset_centromere$query_left <- as.integer(ARS_UOA_Gaur_1_subset_centromere$query_left)

ARS_UOA_Gaur_1_subset_centromere_fil <- ARS_UOA_Gaur_1_subset_centromere %>% 
  filter(query_begin < 100000 | query_left < 100000) %>% 
  group_by(query_sequence) %>% summarise(count = n()) %>% arrange(as.integer(query_sequence))
nrow(ARS_UOA_Gaur_1_subset_centromere_fil)
...
##### <START> telomeric repeats #####
arsucd1_2_subset_telomere <- arsucd1_2_subset %>% filter(`repeat` == "(TTAGGG)n") %>% 
  filter(query_sequence != "Unplaced") %>% filter(query_align_len >= 12)
arsucd1_2_subset_telomere

ARS_UOA_Gaur_1_subset_telomere <- ARS_UOA_Gaur_1_subset %>% filter(`repeat` == "(TTAGGG)n") %>% 
  filter(query_sequence != "Unplaced") %>% filter(query_align_len >= 12)
ARS_UOA_Gaur_1_subset_telomere
#it seems none of these is close to the end of sequence. So I'd consider bad telomere assembly and
#won't analyse this part further. 
